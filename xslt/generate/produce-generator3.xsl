...
<!-- Output:  An XSLT -->
    ...
    <xsl:template match="define-assembly" mode="initial-template">
        <XSLT:template priority="5" expand-text="true"
    ...
    <!-- in these modes these should not match but just in case   -->
    ...
    <xsl:template name="boilerplate">
        ...
        <XSLT:template
            ...
            <XSLT:message>
                <xsl:text expand-text="true">Invoke with parameter make=({/METASCHEMA/define-assembly/root-name => string-join('|') })</xsl:text>
            </XSLT:message>
        </XSLT:template>
        <xsl:comment> - = + = - = # = - = + = - = # = - = + = - = # = - = + = - = # = - = + = - = # = - = + = - = # = </xsl:comment>
    </xsl:template>
