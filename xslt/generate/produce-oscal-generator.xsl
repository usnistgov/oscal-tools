...
<!-- Output:  An XSLT -->
    ...
    <xsl:template name="boilerplate">
        ...
        <xsl:comment>Alternatively, the same results can be produced without stated inputs or parameters, by invoking the appropriate template by name "make-catalog" (etc.), as initial template. This is sometimes more convenient for example when calling the XSLT programmatically.</xsl:comment>
        ...
        <xsl:comment> - when a model permits a choice, only the first defined or referenced of the permitted elements is included. Delete this element to permit alternate members of the choice. </xsl:comment>
        <xsl:comment> - all (permitted</xsl:comment>
